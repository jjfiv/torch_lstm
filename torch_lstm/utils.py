...
class EachReprLSTM(torch.nn.Module):
    """ This module represents a LSTM that takes in a bunch of input and produces 1 single output per input token. """
    ...
    def __init__(
        ...
        super(EachReprLSTM, self).__init__()
        self.device = device
        self.lstm = torch.nn.LSTM(
            input_dim,
            output_dim,
    ...
    def forward(self, xs: List[List[torch.Tensor]]) -> torch.Tensor:
        output, _ = self.lstm(xs)  # TODO: packing
        return output
