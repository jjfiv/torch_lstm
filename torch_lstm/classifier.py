...
class SequenceClassifier(nn.Module):
    def __init__(
        ...
        if char_dim > 0:
            ...
            self.char_lstm = nn.LSTM(char_dim, char_lstm_dim, batch_first=True, bidirectional=True)
        ...
        if config.embeddings:
            ...
            self.word_embed.weight.data.copy_(torch.from_numpy(config.embeddings.vectors))
